		...
		start() {
			...
			if (!lib.config.new_tutorial) {
				...
				var step4 = lib.genAsync(function* () {
					...
					ui.click.menuTab("选项");
					Click_Left_Menu("通用");
					yield new Promise(resolve => ui.controls[0].replace("如果你感到游戏较卡，在选项-通用-流畅模式中，可以开启流畅模式", resolve));
					ui.click.menuTab("选项");
					Click_Left_Menu("外观");
					yield new Promise(resolve => ui.controls[0].replace("在选项-外观-介面缩放中，可以设置介面大小", resolve));
					ui.click.menuTab("选项");
					Click_Left_Menu("技能");
					yield new Promise(resolve => ui.controls[0].replace("在选项-技能中，可以设置自动发动或双将禁配的技能", resolve));
					ui.click.menuTab("武将");
					yield new Promise(resolve => ui.controls[0].replace("在武将中，单击武将可以将其禁用", resolve));
					ui.click.menuTab("卡牌");
					yield new Promise(resolve => ui.controls[0].replace("在卡牌中，单击卡牌可以将其禁用", resolve));
					ui.click.menuTab("其它");
					Click_Left_Menu("命令");
