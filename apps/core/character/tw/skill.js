...
const skills = {
	...
	twgongsun: {
		...
		async cost(event, trigger, player) {
			const result = await player
				...
				.forResult();
			if (!result || !result?.bool || !result.links?.length || !result.targets?.length) {
				return;
			}
			event.result = {
				bool: true,
			...
			while (cards.length) {
				...
				if (cards.length === 1) {
					...
					const buttonResult = await player
						...
						.forResult();
					if (!buttonResult || !buttonResult.bool || !buttonResult.links) {
						return;
					}
	...
	twhuzhu: {
			...
			if (target.isDamaged() && target.hp <= player.hp) {
				const recoverResult = await player
					.chooseBool({
						...
						ai: () => get.recoverEffect(target, player, player) && get.attitude(target, player) > 0,
					})
					.forResult();
		...
		ai: {
			...
			result: {
				target(player, target) {
					const eff = target.isDamaged() && get.attitude(target, player) > 0 ? get.recoverEffect(target, player, target) : 0;
			...
			defend: {
				...
				async content(event, trigger, player) {
					...
					const result = await source
						...
						.forResult();
					if (!result || !result?.bool || !result.cards?.length) trigger.getParent()?.excluded.add(player);
				},
			},
		},
	},
