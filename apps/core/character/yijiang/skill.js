	...
	qianxi: {
		...
		async content(event, trigger, player) {
			...
			let result = await player
				...
				.forResult();
			if (!result || !result.bool || !result.cards?.length || !game.hasPlayer(current => current != player && get.distance(player, current) <= 1)) {
				return;
			}
	...
	oldqiuyuan: {
		...
		async content(event, trigger, player) {
			...
			const result = await target
				...
				.forResult();
			if (!result || !result?.bool || !result?.cards?.length || get.name(result.cards[0], target) !== "shan") {
				trigger.getParent().targets.push(target);
